...
# 周辺のスポットを取得（2km）
def get_nearby_spots(db, current_lat: float, current_lon: float):
    # 地球の半径（km）
    EARTH_RADIUS = 6371
    ...
    tourist_near_spots = (
        select(
            Tourist_spots.id, 
            literal('tourist').label('spot_type'),
            Tourist_spots.name, 
    ...
    gourmet_near_spots = (
        select(
            Gourmet_spots.id, 
            literal('gourmet').label('spot_type'),
            Gourmet_spots.name, 
            distance_expr.label('distance')
        ).where(
            distance_expr <= limit_km,  # 3km以内
    ...
    # 観光地と周辺のSQLを合わせる
    near_union = union_all(tourist_near_spots, gourmet_near_spots)
    
    # グルメの周辺のスポットを取得して返す
    return db.execute(near_union).mappings().all()